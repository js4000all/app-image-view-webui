...
`docs/agent-handoff/current.md` の単一上書きは**廃止**します。

今後は、情報を次の2種類に分離して管理します。

1. **恒久情報（構造・責務・運用ルール）**
   - 保存先: 各ディレクトリの `README.md`（例: `frontend/README.md`, `app/services/README.md`）
   - 内容: 「次の担当者が毎回必要とする前提知識」
2. **タスク固有情報（判断・未解決・検証ログ）**
   - 保存先: `docs/agent-handoff/tasks/YYYY-MM-DD-<topic>.md`
   - 内容: そのタスクでのみ有効な判断、失敗ログ、次アクション

### 記録タイミング

- 1タスク（開始〜完了）ごとに、対象タスクファイルへ追記する。
- 同一タスクでは**追記のみ**（全面置換しない）。
...
コンテキスト引き継ぎ用途で作成してよいファイルは次に限定する。

- `docs/agent-handoff/README.md`（運用ルール・更新手順）
- `docs/agent-handoff/current.md`（運用方針の要約。タスクログ用途では使わない）
- `docs/agent-handoff/tasks/YYYY-MM-DD-<topic>.md`（タスク単位ログ）
- `docs/agent-handoff/archive/YYYY-MM-DD-<topic>.md`（完了タスクの履歴）
- 各ディレクトリの `README.md`（恒久的な責務・仕様の引き継ぎ）

運用ルール:

1. タスクログは `tasks/` に分離し、並列タスク間で同一ファイルを編集しない。
2. 引き継ぎ本文は「事実・判断・未解決」を分離して記述する。
3. 恒久情報へ昇格すべき内容は、タスク完了時に該当ディレクトリ `README.md` へ反映する。
...
※ 一時的な判断は `docs/agent-handoff/tasks/*.md`、恒久化すべき判断は該当ディレクトリ `README.md` に残す。
...
そのため、恒久情報とタスク情報を分離し、競合しない記録運用を行うことを推奨します。
