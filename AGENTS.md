...
複数回のやり取りや途中中断を想定し、**タスクを実行するたびに**作業記録を残す。

- 記録タイミング: タスク開始〜完了の1サイクルごと（小さな修正でも省略しない）
- 記録先: `docs/agent-handoff/current.md`（運用ルールは次節を参照）
...
1. 一時メモを残す場合も、**各タスク実行ごとに**最終的な記録を `current.md` へ統合し、散在させない。
